...
export default async function ServicePage(props: { params: Promise<{ id: string }> }) {
    ...
    // Map imageUrl to image if needed for compatibility with the rest of the component
        ...
        <main className="min-h-screen bg-[#FDF8F3] relative overflow-hidden">
            ...
            <section className="relative h-[80vh] w-full overflow-hidden flex items-end pb-24 px-6 md:px-12 bg-stone-900">
                <div className="absolute inset-0 z-0">
                    <Image
                        src={displayService.image || '/placeholder-service.jpg'}
                ...
                <div className="relative z-10 max-w-7xl mx-auto w-full">
                    ...
                    <h1 className="text-6xl md:text-[10rem] font-light text-stone-900 leading-[0.85] tracking-tighter mb-8 text-stone-900">
                        {displayService.title.split(' ')[0]} <br />
                        <span className="font-serif italic text-amber-600">{displayService.title.split(' ').slice(1).join(' ')}</span>
                    </h1>
                </div>
            </section>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-24">
                        ...
                        <div className="lg:col-span-8">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-24">
                                {(displayService.features || []).map((feature: any, i: number) => (
                                    <div key={i} className="group p-10 rounded-[3rem] bg-white border border-stone-100 shadow-sm hover:shadow-xl transition-all duration-500">
                                        ...
                                        <h3 className="text-xl font-medium text-stone-900 mb-4">{feature.title || feature}</h3>
                            ...
                            {/* Included List */}
                            {displayService.details?.included && (
                                <div className="bg-white rounded-[4rem] p-12 md:p-20 shadow-sm border border-stone-100">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
                                        {displayService.details.included.map((item: string, i: number) => (
                            ...
                            <div className="sticky top-40 space-y-8">
                                <div className="bg-stone-900 rounded-[3rem] p-12 text-white shadow-2xl">
                                    ...
                                    <div className="space-y-12">
                                        {displayService.details?.approach && (
                                            <div>
                                                ...
                                                <p className="text-lg font-light text-stone-200 leading-relaxed">{displayService.details.approach}</p>
                                            </div>
                                        )}

                                        {displayService.details?.timeline && (
                                            <div>
                                                <div className="flex items-center gap-3 text-white/40 mb-3">
                                                    <Clock size={16} />
                                                    <span className="uppercase text-[10px] tracking-widest font-bold">Timeline</span>
                                                </div>
                                                <p className="text-lg font-light text-stone-200 leading-relaxed">{displayService.details.timeline}</p>
                                            </div>
                                        )}

                                        {displayService.details?.suitableFor && (
                                            <div>
                                                <div className="flex items-center gap-3 text-white/40 mb-3">
                                                    <Users size={16} />
                                                    <span className="uppercase text-[10px] tracking-widest font-bold">Suitable For</span>
                                                </div>
                                                <p className="text-lg font-light text-stone-200 leading-relaxed">{displayService.details.suitableFor}</p>
                                            </div>
                                        )}
                                    </div>
                                ...
                                {displayService.details?.materials && (
